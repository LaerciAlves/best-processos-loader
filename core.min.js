// ======================================
// 🧾 LOG GLOBAL (ETAPA 21) — buffer + DEV-only output
// ======================================
...
//(function () {
//  "use strict";
// alert("🔥 SCRIPT CAF CARREGADO");   // 👈 ADICIONE ESTA LINHA AQUI

(function iniciarCAFCore() {
  function bootCAF() {
    // ======================================
    // 🔧 NAMESPACE GLOBAL DO PROJETO
    // ======================================
    ...
    // ======================================
    // 🧾 CAF ENCERRADAS (LOCALSTORAGE) — ETAPA 11
    // ======================================
    ...
    // ======================================
    // 🔐 DEV AUTH FLAG (ETAPA 19.D) — com persistência por ABA (sessionStorage)
    // ======================================
    CAF.dev = CAF.dev || {};
    CAF.dev.auth = CAF.dev.auth || { allowed: false };

    const DEV_FLAG_KEY = "CAF_DEV_ALLOWED"; // só flag, NÃO guarda senha

    // restaura autorização DEV ao recarregar (na mesma aba)
    try {
    ...
    // ======================================
    // 🧪 CONTROLE GLOBAL DE LOG (ETAPA 4)
    // ======================================

    // 🔐 padrão: logs DESLIGADOS para usuários
    CAF.logs.enabled = false;

    CAF.logs.enable = function () {
      // ✅ ETAPA 19.D — bloqueia enable manual sem autorização
      ...
      console.warn("🧪 CAF DEV MODE ATIVADO");

      // ✅ ETAPA 21 — flush logs que ocorreram antes do DEV
      try {
    ...
    CAF.logs.disable = function () {
      ...
      console.warn("🔒 CAF DEV MODE DESATIVADO");

      return true; // 🔥 agora retorna status
    };

    // ======================================
    // 🔁 AUTO-RESTORE DEV LOG (ETAPA 21)
    // roda uma vez ao carregar, não entra em loop
    // ======================================
    try {
      if (CAF.dev && CAF.dev.isAuthorized && CAF.dev.isAuthorized()) {
        // chama enable uma única vez
        CAF.logs.enable();
      }
    } catch (e) {}

    // ======================================
    // 🕵️ DEV PROMPT (ETAPA 19.A) — trigger oculto + modal
    // ======================================
  ...
  padding: 10px;              /* ✅ nunca encosta na borda */
  box-sizing: border-box;
}

#cafDevPromptOverlay, #cafDevPromptOverlay *{
  box-sizing: border-box;     /* ✅ evita “estourar” por padding/borda */
}

#cafDevPromptCard{
  width: clamp(340px, 92vw, 460px);  /* ✅ maior, mas nunca passa do viewport */
  background: #0b1220;
  color: #fff;
...
/* ✅ input mais “curto” (altura/padding menor), mas sempre cabe */
.cafDevRow input{
  width: 100%;
  max-width: 100%;
  padding: 6px 10px;          /* ✅ reduz “comprimento visual” */
  height: 32px;               /* ✅ input mais baixo */
  border-radius: 10px;
...
.cafDevActions{
  ...
  margin-top: 10px;
  flex-wrap: wrap;            /* ✅ se ficar apertado, quebra linha sem estourar */
}

.cafDevBtn{
  padding: 7px 10px;
  height: 32px;               /* ✅ combina com input */
  border-radius: 10px;
  border: 1px solid rgba(255,255,255,.16);
...
.cafDevBtnPrimary{
  ...
  border-color: rgba(255,255,255,.08);
    ...
    CAF.dev.ensureUI = function () {
      ...
      <div id="cafDevPromptHint">⚠️ Credenciais não são armazenadas.</div>
    ...
    CAF.dev.submit = async function () {
      try {
        ...
        // não persiste credencial em lugar nenhum
        if (!user || !pass) {
          if (CAF.logs && CAF.logs.enabled) {
            console.warn("DEV prompt: preencha usuário e senha.");
          }
          return false;
        }
        ...
        if (typeof CAF.dev.onSubmit === "function") {
          ...
          if (CAF.logs && CAF.logs.enabled) {
            console.warn(
              "DEV prompt: handler CAF.dev.onSubmit ainda não configurado (Parte B).",
            );
          }
        }
    ...
    CAF.dev.installHotkey = function () {
      try {
        ...
        document.addEventListener(
          ...
          (ev) => {
            try {
              // não disparar dentro de input/textarea
              const tag =
    ...
    CAF.dev.onSubmit = async function (user, pass) {
      try {
        ...
        if (res && res.ok && res.devAllowed === true) {
          // ✅ ETAPA 19.D — libera enable apenas após validação API
          CAF.dev.setAuthorized(true);

          CAF.logs.enable(); // agora passa pelo gate
          if (CAF.logs && CAF.logs.enabled) {
            console.warn("🧪 DEV liberado via API (master).");
          }
          return true;
        }
        ...
        if (CAF.logs && CAF.logs.enabled) {
          ...
          // se DEV ainda não está ligado, só um alert discreto
          alert("DEV negado.");
        }
    ...
    // ======================================
    // 🔐 AUTH (ETAPA 20.1) — usuário logado + mock DEV
    // ======================================

    CAF.auth = CAF.auth || {};

    // chave só para DEV (não usar em produção)
    CAF.auth.MOCK_KEY = "CAF_DEV_MOCK_USER";

    CAF.auth.setMockUser = function (user) {
      try {
        if (!(CAF.logs && CAF.logs.enabled)) return false; // ✅ só DEV
    ...
    // tenta capturar o usuário logado via DOM (placeholder robusto)
    // depois, quando você estiver no site, ajustamos os seletores reais
    CAF.auth.getLoggedUser = function () {
      try {
        ...
        // 2) Tentativas genéricas (você pode ajustar depois)
        const candidates = [];

        // exemplos comuns: topo com nome, avatar, menu usuário
        const sel = [
          "[data-user]",
          "[data-username]",
          ".user-name",
    ...
    // ======================================
    // 🧪 DEV LOG EXPORT + SNAPSHOT (DEV ONLY)
    // ======================================
    ...
    CAF.logs.buildDebugPayload = function () {
      ...
      // 1) tenta pegar da variável em memória (mais fiel)
      try {
      ...
      // 2) fallback: sessionStorage (entre páginas)
      if (!logs || !logs.length) {
        try {
          logs =
    ...
    // salva snapshot antes de navegar (para não perder rastreio)
    ...
    // botão DEV flutuante (só aparece com DEV ligado) — BLINDADO
    CAF.logs.ensureDevDownloadButton = function () {
      try {
        ...
        setImp(dock, "z-index", "2147483647"); // máximo prático
        ...
        btnJson.type = "button";
        btnJson.textContent = "⬇️ Debug JSON";
    ...
    // ================================
    // 🔥 CONTROLE DE CONTINUIDADE ENTRE PÁGINAS
    // ================================
    ...
    async function validarNCsEstoqueBase(cafid) {
      LOG("🔎 Validando NCs da CAF:", cafid);
      ...
      for (const item of encoids) {
        ...
        if (!status || !status.toLowerCase().includes("estoque da base")) {
          LOG("❌ AWB ainda não tratada:", item.awb);

          return false;
        }
      }

      LOG("✅ Todas NC tratadas (Estoque da Base)");
    ...
    function garantirPaginacao100() {
      ...
      if (select.value !== "100") {
        LOG("⚠️ Paginação não está em 100 — ajustando automaticamente");
    ...
    function pegarCAFInicialDaTela() {
      ...
      for (const row of linhas) {
        ...
        // 🔥 ESTE É O BOTÃO CERTO QUE ABRE O MODAL
        const btnEncerrar =
          tds[21]?.querySelector("a") || tds[22]?.querySelector("a");

        if (btnEncerrar) {
          LOG("✅ Botão ENCERRAR encontrado para contexto inicial");

          return btnEncerrar;
        }
      }

      LOG("❌ Nenhum botão encerrar encontrado na tabela");
      return null;
    }
...
left:8px;                 /* 🔥 distância da lateral */
right:8px;                /* 🔥 mantém dentro da tela */
bottom:8px;               /* 🔥 distância do rodapé */

width:auto;               /* 🔥 evita estourar largura */
...
border-radius:8px;        /* 🔥 aparência moderna */
...
`;
    ...
    // 🔥 NÃO COBRIR RODAPÉ ICS (SAFE DOM READY)
...
flex-wrap:wrap;
`;
    const badgeElegiveis = criarBadge("Elegíveis:0");
    ...
    const badgeMedio = criarBadge("Médio:—");
...
overflow-y:hidden;
max-width:55vw;       /* 🔥 nunca passa metade da tela */
padding-bottom:2px;
`;
    ...
    function criarBtn(txt, cor) {
      ...
      b.style = `
        padding:4px 10px;          /* 🔥 controla altura automaticamente */
        font-size:11px;
        line-height:1.2;
        ...
        display:flex;              /* 🔥 melhor que inline-flex aqui */
        align-items:center;
        justify-content:center;
    ...
    const btnXlsx = criarBtn("Elegíveis", "#0069d9");
    const btnVisao = criarBtn("Visão", "#17a2b8");
    //const btnExportItems = criarBtn("Itens","#6f42c1");
    //const btnExportStats = criarBtn("Stats","#20c997");
    ...
    let MAPA_PLANILHA_CACHE = null; // 🔥 cache da classificação da planilha
    /* =========================
   CONTROLE PAGINAÇÃO CAF
========================= */

    let pageAtual = 0; // 🔥 sempre inicia na primeira página
    const LIMIT_PAGINA = 100; // padrão do sistema

    // 🔁 tenta recuperar planilha após reload/paginação
    try {
      ...
      if (cachePlanilha) {
        ...
        LOG("♻️ Planilha restaurada automaticamente da sessão");
      }
    } catch (e) {
    ...
    function atualizarProgressoUI() {
      ...
      badgeElegiveis.innerText = `Elegíveis:${totalElegiveis}`;
      ...
      badgeMedio.innerText = `Médio:${media ? formatTempoSegundos(media) : "—"}`;
    ...
    /* ========= gerar lista elegíveis (mantido) ========= */
    async function gerarXlsxElegiveis() {
      const linhas = obterLinhasValidas();
      const dados = [];
      dados.push([
        "CAF",
        "QTDE Total",
        "Entregues/RMA/Reversa",
        "Evolução",
        "Link CAF",
      ]);
      for (const row of linhas) {
        const tds = row.querySelectorAll("td");
        const caf = tds[0].innerText.trim();
        const qtdeTotal = tds[6].innerText.trim();
        const entregues = tds[7].innerText.trim();
        const evolucao = tds[19].innerText.trim();
        const botao = tds[21].querySelector("a");
        if (qtdeTotal === entregues && evolucao.includes("100") && botao) {
          const link = tds[0].querySelector("a")?.href || "";
          dados.push([caf, qtdeTotal, entregues, evolucao, link]);
        }
      }
      if (dados.length <= 1) {
        alert("Nenhuma CAF elegível encontrada para gerar XLSX.");
        return;
      }
      const XLSX = await loadXLSX();
      const filename = `caf_elegiveis_${new Date().toISOString().slice(0, 19).replace(/[:T]/g, "-")}`;
      if (XLSX) {
        const wb = XLSX.utils.book_new();
        const ws = XLSX.utils.aoa_to_sheet(dados);
        XLSX.utils.book_append_sheet(wb, ws, "CAF_Elegiveis");
    ...
    btnXlsx.addEventListener("click", async () => {
      ...
      btnXlsx.innerText = "Gerando...";
      await gerarXlsxElegiveis();
      btnXlsx.disabled = false;
      btnXlsx.innerText = "Export CAF's Elegivéis";
    });
    ...
    function registrarLogObj(caf, step, result, detail = "") {
      ...
      // 🔥 LOG INTERNO ORIGINAL (NÃO ALTERADO)
      ...
      // 🔥 NOVO BLOCO LOGGER ESTRUTURADO (ETAPA 4)
      try {
        CAF.logs.step({
          caf,
          ts,
      ...
      // 🔐 CONSOLE LOG SOMENTE SE DEV MODE ATIVO
      if (CAF.logs && CAF.logs.enabled) {
        console.log(
      ...
      // ✅ ETAPA 17 — marca CAF encerrada SOMENTE no sucesso final
      try {
        ...
        if (isFinalSuccess) {
          ...
          // 🔎 DEV MODE: mostra o que foi gravado
          if (CAF.logs && CAF.logs.enabled) {
            console.log("✅ CAF marcada como encerrada (FINAL):", caf, meta);
          }
        }
      } catch (e) {}
    }
    ...
    async function exportarStats() {
      ...
      const aoa = [
        ["Métrica", "Valor"],
        ["CAF elegíveis (no início)", total],
        ["CAF processadas", processed],
        ["Tempo médio por CAF (s)", media.toFixed(2)],
    ...
    // btnExportStats.addEventListener("click", async () => { btnExportStats.disabled = true; btnExportStats.innerText = "Exportando..."; await exportarStats(); btnExportStats.disabled = false; btnExportStats.innerText = "Exportar estatísticas (XLSX/CSV)"; });
    ...
    // ========= Função de scroll automático (para carregar botões e itens dos modais) =========
    ...
    /* ========= ajuda para calcular timeout dinâmico ========= */
    function calcularTimeoutPorItens(itemCount) {
      // base + extra por item (em ms)
      const base = 1000; // mínimo
      const perItem = 150; // ms por item (ajustável)
    ...
    async function processarModal1(caf) {
      ...
      if (!btn) {
        registrarLogObj(
          ...
          "Modal 1",
          "Falha",
          "Botão Encerrar não encontrado (modo v10)",
        );
    ...
    async function processarModal2(caf) {
      ...
      if (!tabela) {
        registrarLogObj(
          ...
          "Modal 2",
          "Falha",
          "Tabela de checkboxes não encontrada",
        );
        return { skipped: false, ok: false, reason: "tabela_nao_encontrada" };
      }

      // 🔥 NOVO — esperar linhas reais carregarem (AJAX)
      ...
      if (!linhasCarregadas) {
        ...
        // 🔥 retry automático da própria função
        return await processarModal2(caf);
      }

      // força scroll e espera dinâmica baseada em qtd de itens (se já capturada preliminarmente usa isto)
      await autoScrollModal();
      await wait(400);

      // tenta pegar número de itens pelo DOM (melhor fonte)
      let itens = coletarItensModal2();
      let itemCount = itens && itens.length ? itens.length : 0;
      // se não achou, tenta checar capturas prévias (quando carregaEncomendasSelecionadas foi clicada)
      if (
        !itemCount &&
      ...
      // 🔥 NOVO — tempo mínimo para AJAX independente da qtd de itens
      ...
      // recolhe itens novamente (após espera)
      ...
      if (itens && itens.length > 0) {
        ...
        await wait(2000);
        return await processarModal2(caf); // retry automático
      }

      // checa se há checkboxes e se estão todos marcados
      ...
      // espera até botão aparecer (mais tentativas quando muitos itens)
      let btnEncSel = null;
      ...
      for (let i = 0; i < Math.min(60, maxLoops * 8); i++) {
        ...
        // 🔥 garante que não pegou botão antigo invisível
      ...
      if (!btnEncSel) {
        registrarLogObj(
          ...
          "Modal 2",
          "Falha",
          "Botão Encerrar Selecionadas não encontrado",
        );
        return {
          skipped: false,
          ok: false,
    ...
    async function processarModal3(caf) {
      ...
      while (t < maxTent && !btnFinalizar) {
        ...
        if (modalContent) {
          ...
          // 🔥 garante que pegou o botão VISÍVEL do modal atual
      ...
      if (!btnFinalizar) {
        registrarLogObj(
          ...
          "Modal 3",
          "Falha",
          "Botão Finalizar Caf não encontrado",
        );
        return {
          skipped: false,
          ok: false,
      ...
      // 🔥 SCROLL CORRETO DO NYROMODAL
      await autoScrollModal();
      await wait(400);

      try {
        // garante que o botão esteja dentro do viewport do modal
    ...
    async function processarModal4_tryClose(caf) {
      ...
      registrarLogObj(caf, "Modal 4", "Falha", "Não conseguiu fechar modal");
    ...
    async function processarFluxoModaisParaCAF(numeroCAF) {
      ...
      if (!wrapper) {
        registrarLogObj(numeroCAF, "Fluxo", "Falha", "Modal wrapper não abriu");
        return { ok: false };
      }
      ...
      // 🔥 VERIFICA PELO CONTROLE INTERNO DO SCRIPT
      ...
      if (aindaTemElegivel) {
        registrarLogObj(
          ...
          "Fluxo",
          "Continuar",
          `Ainda existem CAFs elegíveis (${processadas + 1}/${totalElegiveis}), não fecha modal final`,
        );
    ...
    async function processarFluxoCAFZerada(numeroCAF) {
      const wrapper = await waitForSelector("#nyroModalWrapper", 4000);
      if (!wrapper) {
        registrarLogObj(numeroCAF, "Fluxo Zerada", "Falha", "Modal não abriu");
        return { ok: false };
      }
      ...
      return new Promise((resolve) => {
        ...
        reader.onload = function (e) {
          try {
            ...
            // 🧠 salva planilha para sobreviver paginação
            try {
              sessionStorage.setItem("CAF_PLANILHA", JSON.stringify(linhas));
              LOG("💾 Planilha salva em sessionStorage");
            } catch (e) {
            ...
            console.error("❌ ERRO lendo planilha local:", err);
            resolve([]);
          }
        };
    ...
    function classificarCAFPlanilha(linhas) {
      ...
      const normalizarEvolucao = (val) => {
        ...
        // 🔥 Excel retorna porcentagem como decimal (1 = 100%)
      ...
      for (let i = 1; i < linhas.length; i++) {
        ...
        const evolucao = normalizarEvolucao(row[19]); // Evolução
    ...
    async function analisarResumoPlanilha() {
      ...
      const msg = `📊 RESUMO DA PLANILHA

CAF Elegíveis: ${totalElegiveis}
Total itens elegíveis: ${itensElegiveis}
...
`;
      ...
      // 🔥 janela copiável igual você já usa
      const box = document.createElement("div");
      box.style = `
        position:fixed;
        left:50%;
    ...
    async function processarLinhas() {
      ...
      if (!periodo) {
        ...
        LOG("🚀 Primeira carga controlada:", primeiraURL);
      ...
      try {
        ...
        if (cacheLogs) {
          ...
          LOG("♻️ Logs restaurados:", logsProcessamento.length);
        }
      } catch (e) {}
      ...
      for (const row of linhas) {
        ...
        // ✅ ETAPA 16 — se estiver em modo "target", processa só as CAFs da lista
        try {
        ...
        // 🔥 NÃO PROCESSA DUPLICADO
        if (jaProcessouCAF(numeroCAF)) {
          LOG("⏭️ CAF já processada anteriormente:", numeroCAF);
          continue;
        }
      ...
      // ==================================
      // 🔥 CONTROLE INTELIGENTE DE FINALIZAÇÃO
      // ==================================
      ...
      // 🔴 FINALIZA SE TODAS CAF PROCESSADAS
      if (todasProcessadas) {
        LOG("🏁 Todas CAF elegíveis da planilha foram processadas.");
        ...
        // 🧪 DEV: snapshot antes de limpar logs
        try {
          if (CAF.logs && CAF.logs.enabled) {
            CAF.logs.saveSnapshotBeforeUnload();
          }
        ...
        // ✅ ETAPA 13 — verificação final de pendentes (somente LOG)
        try {
          ...
          // ✅ ETAPA 14 (fase 1) — salva pendentes para uso futuro
          CAF.engine.salvarPendentes(pend);

          // marca que ainda NÃO reprocessou nesta execução
          CAF.engine.setReprocessDone(false);

          await CAF.engine.tryAutoReprocess();

          if (CAF.logs && CAF.logs.enabled) {
            console.log("💾 Pendentes salvas (sem reprocessar):", pend.length);
          }
        } catch (e) {}

        // ✅ ETAPA 16 — reprocessa automaticamente (uma vez) se houver pendentes
        try {
      ...
      // 🔵 FINALIZA SE CHEGOU NA ÚLTIMA PÁGINA
      if (paginaIndexAtual >= totalPaginas - 1) {
        LOG("🏁 Última página baseada na planilha atingida.");
      ...
      if (stopRequested) {
        LOG("🛑 Stop solicitado — não irá para próxima página");
      ...
      LOG("➡️ Indo para próxima página:", novaURL);
    ...
    CAF.engine.startNormais = async function () {
      ...
      if (!startSaved) {
        ...
        LOG("⏱️ Início global salvo:", startSaved);
      }
      ...
      // ====== (controle paginação / estado) ======
      sessionStorage.setItem(AUTO_PAGINA_KEY, "0");
      sessionStorage.removeItem(AUTO_PROCESSADAS_KEY);

      // 🔥 MUITO IMPORTANTE (mantido do seu fluxo)
      sessionStorage.removeItem(AUTO_STATE_KEY);
    ...
    // ======================================
    // 🔁 RESUME ENGINE (ETAPA 10 - CORRIGIDA)
    // ======================================
    ...
    CAF.engine.resumeIfNeeded = async function () {
      try {
        ...
        LOG("🚀 Retomando processamento automaticamente (pós paginação)");

        // 🔥 UI (mantém exatamente como era antes)
        btnStart.disabled = true;
        btnStart.style.background = "#777";
        btnStart.innerText = "Processando...";
    ...
    CAF.engine.detectarPendentes = function (listaOriginal) {
      try {
        ...
        if (CAF.logs && CAF.logs.enabled) {
          console.log("📋 Pendentes detectadas:", pendentes);
        }
    ...
    // ======================================
    // 🧠 VERIFICAR PENDENTES AO FINAL (ETAPA 13)
    // ======================================
    ...
    CAF.engine.verificarPendentesAoFinal = function (listaElegiveis) {
      try {
        ...
        if (CAF.logs && CAF.logs.enabled) {
          console.log(
            "📊 Verificação final — elegíveis:",
            report.elegiveisTotal,
          );
          console.log(
            "📊 Verificação final — pendentes:",
            report.pendentesTotal,
          );

          if (pendentes.length) {
            console.warn("⚠️ CAFs pendentes detectadas:", pendentes);
          } else {
            console.log("✅ Nenhuma CAF pendente.");
          }
        }
    ...
    // ======================================
    // 🎯 TARGET LIST (PENDENTES) — ETAPA 16
    // ======================================
    ...
    // ======================================
    // ♻️ REPROCESSAMENTO (PREP) — ETAPA 14 FASE 1
    // ======================================
    ...
    // ======================================
    // ♻️ AUTO REPROCESS FINAL — ETAPA 16
    // ======================================

    CAF.engine.tryAutoReprocessFinal = async function () {
      try {
        // não reprocessa se usuário pediu stop
        ...
        // define lista-alvo e reinicia processamento na mesma página (sem mudar regra)
        CAF.engine.setTargetList(pendentes);

        if (CAF.logs && CAF.logs.enabled) {
          console.warn(
            "♻️ Reprocessamento automático iniciado (target list):",
            pendentes.length,
            "attempt:",
            nextAttempt,
          );
        ...
        // importante: não recria período / não muda página
        await CAF.engine.processarLinhas();
    ...
    // ======================================
    // ♻️ AUTO REPROCESS (ETAPA 14 - FASE 2)
    // ======================================
    ...
    CAF.engine.tryAutoReprocess = async function () {
      try {
        ...
        if (CAF.engine.isReprocessDone()) {
          if (CAF.logs && CAF.logs.enabled) {
            console.log("🔁 Reprocessamento já realizado anteriormente.");
          }
          return false;
        }
        ...
        if (CAF.logs && CAF.logs.enabled) {
          console.warn(
            "♻️ Iniciando reprocessamento automático:",
            pendentes.length,
          );
        }

        // marca como já reprocessado
        CAF.engine.setReprocessDone(true);
    ...
    btnAnalisePlanilha.addEventListener(
      ...
      async (e) => {
        ...
        console.log("🔥 CLICK REAL CAPTURADO");

        try {
          console.log("🔥 chamando analisarResumoPlanilha...");

          await analisarResumoPlanilha();

          console.log("🔥 analisarResumoPlanilha FINALIZOU");
        } catch (err) {
          console.error("❌ ERRO AO EXECUTAR:", err);
        }
      },
      true,
    ); // 🔥 CAPTURE TRUE É O SEGREDO

    btnSelecionarPlanilha.onclick = () => {
    ...
    btnClearStorage.onclick = () => {
      if (!confirm("Deseja limpar TODOS os dados da automação?")) return;
      ...
      alert("Storage limpo. Recarregue a página.");
    };

    /* ========= interceptador de clique para carregaEncomendasSelecionadas(...) =========
       Captura quando o usuário clica no link que chama essa função e extrai numcaf + unidade (quando possível)
       mantém dados preliminares para estimativa antes da abertura do modal.
    */
    document.addEventListener(
      "click",
      (ev) => {
        try {
          let el = ev.target;
          // sobe até encontrar um <a> ou elemento com onclick
          ...
          if (m) {
            const cafNum = m[1];
            const unidade = m[2];
            // armazena captura preliminar (qtde desconhecida por esse onclick, mas marca presença)
            if (!capturasPreliminares[cafNum])
              capturasPreliminares[cafNum] = { expectedItems: 0, unidade };
            // mostra um aviso rápidobadgeCAFAtual.innerText = `CAF:${cafNum}`;
    ...
    /* ========= helpers finais ========= */
    // função para calcular estima completa (usada UI)
    ...
    /* ========= export itens já ligado acima a btnExportItems (mantido) ========= */

    // manter botão XLSX original e export logs via baixarLogXlsx/baixarLogTxt (chamados no final)

    // final
    atualizarContadorUI();

    // ======================================
    // 🔐 HASH GLOBAL DO SCRIPT (BOOT REAL)
    // ======================================
    (async () => {
      try {
        ...
        LOG("🔐 Hash do script carregado:", hash);
      } catch (e) {
    ...
    // ================================
    // 🔥 AUTO CONTINUAÇÃO APÓS PAGINAÇÃO
    // ================================
    setTimeout(async () => {
  ...
  // 🔥 BOOT SEGURO — ESPERA BODY EXISTIR
